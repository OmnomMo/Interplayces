...
public class ShipPartStack : MonoBehaviour {
	...
	void Update () {
        ...
        {
            Ray ray = touchCam.GetComponent<Camera>().ScreenPointToRay(Input.mousePosition);
            RaycastHit hit;
            Debug.Log("Raycast");
            if (Physics.Raycast(ray, out hit))
            {
                if (hit.collider.tag == "DragPart")
                {
                    Debug.Log("Ray hit dragPart");
                }
            }
        }
    }
